document.getElementById('excelFile').addEventListener('change', function (e) {
  ...
  .then(res => {
    console.log("🔥 Đã phản hồi từ server, status:", res.status);
    return res.text(); // lấy dạng text để debug
  })
  .then(text => {
    console.log("📦 Raw response text:", text);
    try {
      const data = JSON.parse(text);
      console.log("✅ Kết quả xử lý (JSON):", data);
    } catch (err) {
      console.error("❌ JSON lỗi:", err.message);
    }
  })
  .catch(err => {
